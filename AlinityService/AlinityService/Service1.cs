...
namespace AlinityService
{
    public partial class Service1 : ServiceBase
    {
        ...
        static object TCPClientLogLocker = new object();   // локер для логов TCP сервера драйвера
        ...
        public void TCPServer()
        {
            try
            {
                while (ServiceIsActive)
                {
                    ...
                    //TCPServerLog("Настройка сокета - socket.SetSocketOption();");
                    ...
                    while (ServiceIsActive)
                    {
                        ...
                        if (client.Available == 0)
                        {
                            ...
                            #region принудительная перезагрузка сокета 
                            // соединение почему-то отваливается каждый день, предположительно тогда же, когда и отваливается сервер на приборе, в районе 2 ночи
                            // клиент прибора теряет соединение, хотя по состоянию сокета все ОК
                            
                            DateTime now = DateTime.Now;
                            // каждый час перезагружаем соединение, не закрывая сокет
                            //if (now.ToString("mm:ss") == "00:00")
                            ...
                            if (now.ToString("HH:mm:ss") == "08:00:00")
                            {
                                ...
                                TCPServerLog("Свойства сокета TCP сервера: " + 
                                             ...
                                             $"RemoteEndPoint: {client.RemoteEndPoint}; " + 
                                             $"LocalEndPoint: {client.LocalEndPoint}; ");
                                TCPServerLog("Перезагрузка соединения");
                                TCPServerLog("Ожидание инициализации соединения от прибора...");
                                client = socket.Accept();

                                TCPServerLog($"Адрес подключенного клиента: {client.RemoteEndPoint}");
                                

                                /*
                                try
                                {
                                    TCPServerLog("Socket.Shutdown().");
                                    client.Shutdown(SocketShutdown.Both);
                                }
                                finally
                                {
                                    TCPServerLog("Закрываем сокет. Socket.Close().");
                                    client.Close();
                                    //Thread.Sleep(5000);
                                }
                                
                                TCPServerLog("Создаем новый сокет");
                        ...
                        else
                        {
                            ...
                            TCPServerLog("Свойства сокета TCP сервера: " + $"Blocking: {client.Blocking}; " + $"Connected: {client.Connected}; " + $"RemoteEndPoint: {client.RemoteEndPoint}; ");
                            // состояние сокета
                            TCPServerLog("Состояние сокета TCP сервера: " +
                                           $"handler.Available {client.Available}; " +
                                           $"SelectRead: {client.Poll(1, SelectMode.SelectRead)}; " +
